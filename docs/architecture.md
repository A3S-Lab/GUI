...
the original tag is preserved as `data-a3s-html-tag` metadata. Text, form,
list, dialog, menu, media, embedded-content, and table-structure tags lower to
dedicated native roles. Generic HTML containers lower to `NativeRole::View`;
unsupported custom elements with a hyphenated tag name also lower to a generic
native view.
