...
| `minLength` / `maxLength` / `rows` / `cols` / `size` | normalized to native numeric control hints and preserved as metadata; `rows`, `cols`, and `size` must be positive integers before they affect native sizing; text-field values are clamped to `maxLength` before initial render, before rerender updates, and when native change events arrive |
...
| `onChange` / `onInput` | normalized to the primary action for change, selection, and value-toggle controls; text-field change values are clamped to `maxLength`, number-input and slider change values are clamped to min/max range bounds and snapped to step hints, checked control change payloads are canonical `true` / `false` strings, invalid numeric change payloads are ignored before state updates or reducer dispatch, selection-container events without payloads infer the current selected child, and `onChange` wins when both are present |
...
selection containers also suppress value-changing events from selectable
descendants.
