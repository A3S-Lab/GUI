...
pub fn controls_frame(state: &ControlsState, frame_id: &str, title: &str) -> GuiResult<UiFrame> {
    ...
    .map_err(|error| GuiError::invalid_tree(format!("invalid controls frame: {error}")))
}

pub fn controls_reduce(state: &mut ControlsState, invocation: &ActionInvocation) -> GuiResult<()> {
    match invocation.action.as_str() {
        ...
        "setVolume" => {
            state.volume = invocation
                ...
                .unwrap_or("0")
                .parse()
                .map_err(|error| GuiError::host(format!("invalid volume: {error}")))?;
        }
        "setTheme" => {
        ...
        other => {
            return Err(GuiError::host(format!("unexpected action {other}")));
        }
    }
    Ok(())
}
